...
void SriteBatch::createRenderBatches() {
	...
	for (int i = 1; i < _glyphs.size(); i++) {
		if (_glyphs[i]->texture != _glyphs[i-1]->texture)
			_renderBatch.emplace_back(offset, 6, _glyphs[i]->texture);
		else
...
void SriteBatch::draw(const glm::vec4& destRest, const glm::vec4& uvRect, GLuint texture, float depth, const Color& color) {
	...
	glyph->topLeft.setPosition(destRest.x, destRest.y + destRest.w);
	...
	glyph->topRight.setPosition(destRest.x + destRest.z, destRest.y + destRest.w);
	glyph->topRight.setUV(uvRect.x + uvRect.z,uvRect.y + uvRect.w);

	glyph->bottomRight.setPosition(destRest.x + destRest.z, destRest.y);
...
void SriteBatch::renderBatch() {
	...
	for (int i = 0; i < _renderBatch.size(); i++) {
		...
		glDrawArrays(GL_TRIANGLES, _renderBatch[i]._offset, _renderBatch[i]._numVertices);
	}
	glBindVertexArray(0);
}
