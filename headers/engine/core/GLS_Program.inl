GLS_Program::GLS_Program(): _programID(0) , _vertexShaderID(0), _fragmentShaderID(0), _numAtribute(0) {
...
void GLS_Program::compileShader(const string& shaderPath, GLuint id) {
	...
	if (isCompiled == GL_FALSE) {
		...
		fatalError("Shaders do not compiled " + printf("%s", &(infoLog[0])));
...
void GLS_Program::addAtribute(const string atributeName) {
	glBindAttribLocation(_programID, _numAtribute++, atributeName.c_str());
}
